...
describe('assembleAsm (fallback engine)', () => {
  describe('basic instructions', () => {
    ...
    it('assembles RET', () => {
      const result = assembleAsm('ret');
      expect(result.bytes).toEqual([0xc3]);
      expect(result.hex).toBe('C3');
    });

    it('assembles INT3', () => {
      const result = assembleAsm('int3');
      expect(result.bytes).toEqual([0xcc]);
    });

    it('assembles NOP; RET; INT3 as multi-instruction', () => {
      const result = assembleAsm('nop; ret; int3');
  ...
  describe('register instructions', () => {
    it('assembles MOV rax, 0x1234', () => {
      const result = assembleAsm('mov rax, 0x1234');
    ...
    it('assembles PUSH rax', () => {
      const result = assembleAsm('push rax');
      expect(result.bytes).toEqual([0x50]);
    });

    it('assembles PUSH rbx', () => {
      const result = assembleAsm('push rbx');
      expect(result.bytes).toEqual([0x53]); // 0x50 + 3
    });

    it('assembles POP rax', () => {
      const result = assembleAsm('pop rax');
      expect(result.bytes).toEqual([0x58]);
    });

    it('assembles XOR rax, rax (zeroing idiom)', () => {
      const result = assembleAsm('xor rax, rax');
      expect(result.bytes[0]).toBe(0x48); // REX.W
      expect(result.bytes[1]).toBe(0x31); // XOR r/m64, r64
      expect(result.bytes[2]).toBe(0xc0); // ModRM: rax, rax
    });

    it('assembles INC rax', () => {
      const result = assembleAsm('inc rax');
      expect(result.bytes[0]).toBe(0x48); // REX.W
      expect(result.bytes[1]).toBe(0xff); // INC r/m64
      expect(result.bytes[2]).toBe(0xc0); // ModRM: /0
    });

    it('assembles DEC rcx', () => {
      const result = assembleAsm('dec rcx');
  ...
  describe('errors and edge cases', () => {
    it('throws on empty input', () => {
      expect(() => assembleAsm('')).toThrow('no instructions');
    });

    it('returns fallback engine flag', () => {
      const result = assembleAsm('nop');
      expect(result.engine).toBe('fallback');
    });

    it('produces warnings for unsupported instructions with fallback placeholder', () => {
      const result = assembleAsm('vaddps ymm0, ymm1, ymm2');
      expect(result.engine).toBe('fallback');
    ...
    it('assembles RET with immediate', () => {
      const result = assembleAsm('ret 0x10');
