...
describe('memory manifest platform filtering', () => {
  ...
  it(`should have ${IS_WIN32 ? 65 : 52} tools on ${process.platform}`, async () => {
    const manifest = await loadManifestWithPlatform();
    const expected = IS_WIN32 ? 65 : 52;
  ...
  it('should honor registry platform override for metadata generation', async () => {
    ...
    expect(win32Manifest.registrations.length).toBe(65);
    ...
    expect(linuxManifest.registrations.length).toBe(52);
  });
});
